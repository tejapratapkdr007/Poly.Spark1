...
app.post("/questions", (req, res) => {
    const { question, answer, answerOpinion, questionFile, questionFileType, questionFileName } = req.body;
    ...
    if (questionFile && questionFileType) {
        try {
            // Detect real extension from the data URL's mime type, or fall back to the
            // original filename's extension, before defaulting to a generic one.
            let ext = null;
            const mimeMatch = questionFile.match(/^data:([^;]+);/);
            if (mimeMatch) {
                const subtype = mimeMatch[1].split('/')[1];
                if (subtype) ext = subtype.split('+')[0]; // e.g. "svg+xml" -> "svg"
            }
            if (!ext && questionFileName && questionFileName.includes('.')) {
                ext = questionFileName.split('.').pop().toLowerCase();
            }
            if (!ext) ext = questionFileType === 'audio' ? 'mp3' : 'jpg';
